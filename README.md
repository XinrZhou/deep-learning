...
$$
y^{(i,j)}:f_{(w,b,x)}=g(w^{(j)}\cdot x^{(i)}+b^{(i)})
$$
...
找到与 $x^{(i)}$ 相似的特征 $x^{(k)}$，计算 $\sum_{l=1}^n(x_l^{(k)}-x_l^{(i)})^2$

**协同过滤算法的限制**

- 处理冷启动问题使用协同过滤结果可能不太准确

- 不能提供自然的方式来使用附加信息

#### 基于内容过滤

协同过滤：根据其他和你评分相近的用户的评分，向你推荐商品

**基于内容过滤**：根据用户和物品的特征，做好匹配后向你推荐物品

- $x_u^{(j)}$: features for user $j$

- $x_m^{(i)}$: features for movie $i$

- 给定特征，判断电影 $i$ 是否能跟用户 $j$ 匹配
- 从 $x_u^{(j)},x_m^{(i)}$ 中计算出特征向量 $v_u^{(j)},v_m^{(i)}$ 
- 计算 $v_u^{(j)}\cdot v_m^{(i)}$

**基于内容过滤的深度学习方法**

- 训练神经网络的参数 $v_u,v_m$ ，输出层维度大小相同
- 计算 $v_u^{(j)}\cdot v_m^{(i)}$
- 代价函数 $J=\sum_{(i,j):r(i,j)=1}(v_u^{(j)}\cdot v_m^{(i)}-y^{(i,j)})^2$+NN regularization term
